...
async function validateApiKey(
  apiKey: string,
  supabase: any,
): Promise<{ valid: boolean; error?: string; apiKeyId?: string }> {
  if (!apiKey) {
    return {
      valid: false,
  ...
  if (count && count >= keyData.rate_limit_per_minute) {
    return {
      valid: false,
      error: `Rate limit exceeded. Maximum ${keyData.rate_limit_per_minute} requests per minute.`,
    }
  }
              ...
              functionDeclarations: [
                {
                  ...
                  description:
                    'Extract structured disaster victim information from raw message',
                  parameters: {
                    type: 'object',
                    properties: {
                      name: { type: 'string' },
