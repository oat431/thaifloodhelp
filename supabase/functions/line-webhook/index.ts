...
// Use messageId as session key - postback data limit is 300 chars
function createReviewFlexMessage(
  report: any,
  reportIndex: number,
  messageId: string,
): any {
  ...
  if (report.name) {
    bodyContents.push({
      ...
      contents: [
        ...
        {
          type: 'text',
          text: report.name + (report.lastname ? ` ${report.lastname}` : ''),
          size: 'sm',
          color: '#111111',
          flex: 5,
          wrap: true,
  ...
  if (totalPeople > 0) {
    ...
    if (report.number_of_adults > 0)
      peopleParts.push(`ผู้ใหญ่ ${report.number_of_adults}`)
    if (report.number_of_children > 0)
      peopleParts.push(`เด็ก ${report.number_of_children}`)
    if (report.number_of_infants > 0)
      peopleParts.push(`ทารก ${report.number_of_infants}`)
    if (report.number_of_seniors > 0)
      peopleParts.push(`ผู้สูงอายุ ${report.number_of_seniors}`)
    if (report.number_of_patients > 0)
      peopleParts.push(`ผู้ป่วย ${report.number_of_patients}`)
  ...
  // Postback data: use messageId as session key + report index
  // Format: "a=s&m=<messageId>&i=<index>" (short keys to save space)
  return {
    type: 'flex',
        ...
        contents: [
          {
            ...
            action: {
              ...
              data: `a=s&m=${messageId}&i=${reportIndex}`,
            },
          },
          {
            type: 'button',
            ...
            action: {
              type: 'postback',
              label: 'ยกเลิก',
              data: `a=c&m=${messageId}&i=${reportIndex}`,
            },
          },
        ],
      },
...
// Session management functions using Supabase
// Use messageId as session key (unique per message)
...
async function getSession(
  ...
  if (diffMinutes > 30) {
    await deleteSession(supabase, messageId)
    return null
  }
...
async function deleteSession(supabase: any, messageId: string): Promise<void> {
  await supabase.from('line_sessions').delete().eq('message_id', messageId)
}
  ...
  try {
    ...
    for (const event of events) {
      ...
      if (event.type === 'message') {
        ...
        const messageId = message.id // Use messageId as session key
        let rawMessage = ''
        ...
        // Handle image message
          ...
          try {
            ...
            const imageBase64 = await getImageContent(
              messageId,
              LINE_CHANNEL_ACCESS_TOKEN,
            )

            // Perform OCR
            rawMessage = await performOCR(
              imageBase64,
              SUPABASE_URL,
        ...
        try {
          ...
          // Save session to database with messageId as key
          await saveSession(supabase, messageId, {
            reports,
            lineUserId: userId,
            lineDisplayName,
          })
          ...
          const flexMessages = reports
            .slice(0, 5)
            .map((report, index) =>
              createReviewFlexMessage(report, index, messageId),
            )
      ...
      // Handle postback events (button clicks)
        ...
        const action = postbackData.get('a') // 's' = submit, 'c' = cancel
        const sessionMessageId = postbackData.get('m') // messageId as session key
        const reportIndex = parseInt(postbackData.get('i') || '0')

        if (action === 's' && sessionMessageId) {
          // Get session from database
          const session = await getSession(supabase, sessionMessageId)
          ...
          try {
            ...
            if (duplicates.length > 0) {
              ...
              const { error: insertError } = await supabase
                ...
                .insert({
                  ...
                  reporter_name:
                    report.reporter_name || session.lineDisplayName || '',
                  address: report.address || '',
                  phone: report.phone || [],
                  location_lat: report.location_lat
                    ? parseFloat(report.location_lat)
                    : null,
                  location_long: report.location_long
                    ? parseFloat(report.location_long)
                    : null,
            ...
            // Mark this report as submitted in session
            session.reports[reportIndex] = null
            const remainingReports = session.reports.filter(
              (r: any) => r !== null,
            )

            if (remainingReports.length === 0) {
              // All reports submitted, delete session
              await deleteSession(supabase, sessionMessageId)
            } else {
              // Update session with remaining reports
              session.reports = remainingReports
              await saveSession(supabase, sessionMessageId, session)
            }
          } catch (error) {
        ...
        } else if (action === 'c' && sessionMessageId) {
          // Cancel - mark report as cancelled in session
          const session = await getSession(supabase, sessionMessageId)
