...
serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      return new Response(JSON.stringify({ error: 'กรุณาระบุที่อยู่' }), {
        status: 400,
...
- "บ้านเลขที่ 45 ถ.สุขุมวิท คลองเตย กทม" → "บ้านเลขที่ 45 ถนนสุขุมวิท แขวงคลองเตย เขตคลองเตย กรุงเทพมหานคร 10110"`
    ...
    if (!completedAddress) {
      return new Response(JSON.stringify({ completedAddress: address }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    return new Response(JSON.stringify({ completedAddress }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  } catch (error) {
